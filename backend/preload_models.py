...
def preload_all_models():
    ...
    # 3. MiniCPM-V 2.6 (4-bit Quantized)
    logger.info("3/3 [Dense Captioning] Preloading MiniCPM-V 2.6 (4-bit quantized)...")
    try:
        captioner = MiniCPMDenseCaptioner()
        captioner._lazy_load()
        if captioner.model is not None:
            logger.info("✅ MiniCPM-V 2.6 4-bit loaded successfully.")
        else:
            logger.warning("⚠️ MiniCPM-V 2.6 loaded with fallback descriptor.")
    except Exception as e:
        logger.error(f"❌ Failed to load MiniCPM-V 2.6: {e}")
