...
export const SearchProfileControl: React.FC<SearchProfileControlProps> = ({ value, disabled = false, onChange }) => (
  <div className="flex items-center gap-1 rounded-xl bg-surface border border-surfaceBorder p-1" role="group" aria-label="Retrieval profile">
    {(["fast", "accurate"] as const).map((profile) => {
      ...
      return (
        <button
          ...
          title={profile === "accurate" ? "SigLIP2 retrieval + automatic SAM/Qwen verification; budget ไม่เกิน 30 วินาที" : "ค้นหาเร็วจาก visual index; warm target ไม่เกิน 1 วินาที"}
        >
    ...
    <span className="sr-only"><Gauge /> {value === "accurate" ? "Qwen visual verification enabled" : "Visual index retrieval enabled"}</span>
  </div>
);
