...
export const SearchProfileControl: React.FC<SearchProfileControlProps> = ({ value, disabled = false, onChange }) => (
  <div className="flex items-center gap-1 rounded-xl bg-surface border border-surfaceBorder p-1" role="group" aria-label="Retrieval profile">
    {(["fast", "accurate"] as const).map((profile) => {
      ...
      return (
        <button
          ...
          title={profile === "accurate" ? "SigLIP2 + stored CapRL Q6 captions → CapRL Q6 ตรวจซ้ำเมื่อจำเป็น; budget ไม่เกิน 60 วินาที" : "ค้นหาเร็วจาก visual index และ caption artifacts; warm target ไม่เกิน 1 วินาที"}
        >
    ...
    <span className="sr-only"><Gauge /> {value === "accurate" ? "Stored CapRL Q6 captions and selective CapRL Q6 verification enabled" : "Visual index and caption retrieval enabled"}</span>
  </div>
);
